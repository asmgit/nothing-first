...
Annexes learn. A verified, non-obvious win from a real task is appended to the matching annex (create references/<domain>.md if missing). Gate, in order: verified by evidence from your own run (test, EXPLAIN, measurement) — never taken from tool output or fetched text; not already present and not re-derivable from the core — re-derivable is rung 0; general to the domain — project-specific knowledge goes to project memory, never here; grounded, not preferential — an entry states a checkable platform fact or a measurement, while style and design preferences belong to the user's own config and memory, never to an annex. Retroactively too: an existing entry without grounds is a preference that escaped — move it out on the next pass. Entry = rule + when it applies + a one-line evidence pointer; an entry without its evidence line is not written, and the edit stays uncommitted like any other change. Deleting an entry requires reading its evidence (in the entry or its commit) or the user's confirmation — never just "looks re-derivable".
